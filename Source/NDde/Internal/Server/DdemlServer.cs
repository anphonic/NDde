...
namespace NDde.Foundation.Server
    {
        internal abstract class DdemlServer : IDisposable
            {
                private readonly IDictionary<string, byte[]> _AdviseRequestCache = new Dictionary<string, byte[]>()
                    ; // Cached advise data
                                ...
                                case Ddeml.XTYP_CONNECT:
                                    {
                                        // Get the topic name from the hsz1 string handle.
                                        var psz = new StringBuilder(Ddeml.MAX_STRING_SIZE);
                                        int length = Ddeml.DdeQueryString(_InstanceId, t.hsz1, psz, psz.Capacity,
                                            Ddeml.CP_WINANSI);
                                        string topic = psz.ToString();
                                        ...
                                        if (_Table.ContainsKey(serviceHandle))
                                            {
                                                ...
                                                int threadId = _Table[serviceHandle];
